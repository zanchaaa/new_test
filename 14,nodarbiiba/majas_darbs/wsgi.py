...
def load_template(name):
    with open(name, 'r') as templete:
        source = string.Template(templete.read())
    return(source)

def read_request_body(environment):
    try:
        request_body_size = int(environment.get('CONTENT_LENGHT', 0))
    except ValueError:
        request_body_size = 0
    request_body = environment['wsgi.input'].read(reaquest_body_size)

    return request_body.decode()

def parse_request_body(request_body):
    parsed = parse.qsl(request_body)
    return dict(parsed)
...
def index(request):
    templete = load_templete('index.html')
    response = Response(content=templete.substitute(),)
    return response
def university_response(request):
    ...
    university_check = named_university.point.check()
    if university_check:
        result = "can"
    else:
        result = "can not"
...
with make_server(host=SERVER_HOST, port=SERVER_PORT, app=application,) as wsgi_server:
